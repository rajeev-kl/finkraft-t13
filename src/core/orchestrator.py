...
from db.crud import create_thread, create_message, create_ai_suggestion, get_or_create_thread, get_message_by_thread_and_body, has_accepted_decision_for_message
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            if cols and 'timestamp' not in cols:
                logger.info("Existing 'email_threads' table missing 'timestamp' column — recreating tables for dev environment")
    ...
    try:
        for i, t in enumerate(threads):
            try:
                ...
                thread_model = get_or_create_thread(db=db, subject=subject, sender=sender, recipient=recipient, body=body)
                ...
                for m in messages:
                    try:
                        ...
                        if existing_msg:
                            ...
                            msg_model = create_message(db=db, thread_id=thread_model.id, sender=m_sender, recipient=m_recipient, body=m_body)
                        ...
                        if has_accepted_decision_for_message(db=db, message_id=msg_model.id):
                            logger.info(f"Skipping re-evaluation for message {msg_model.id} because a suggestion was accepted")
                            continue
                        ...
                        try:
                            if ai_resp is not None:
                                ...
                                if not required_fields_customer and legacy:
                                    ...
                                    try:
                                        required_fields_customer = [{"name": x, "hint": None, "required": True} for x in legacy]
                                    except Exception:
                                        required_fields_customer = None
                        ...
                        try:
                            ...
                            if ai_suggested_action:
                                ...
                                if isinstance(ai_suggested_action, str) and ai_suggested_action.strip() and ai_suggested_action != "no-action":
                        ...
                        logger.info(f"Persisting AISuggestion (provenance={provenance}) for message={msg_model.id} intent={intent} confidence={confidence} action={suggested_action} required_fields_customer={required_fields_customer} required_fields_responder={required_fields_responder} follow_up={follow_up_question}")
                        create_ai_suggestion(db=db, message_id=msg_model.id, intent=intent, confidence=confidence, suggested_action=suggested_action, required_fields=rf_payload, follow_up_question=follow_up_question, raw_response=raw_resp)
