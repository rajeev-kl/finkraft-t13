...
from .models import AISuggestion, EmailMessage, EmailThread, UserDecision
...
def get_thread_by_keys(db: Session, subject: str, sender: str, recipient: str) -> Optional[EmailThread]:
    return (
        ...
        .filter(EmailThread.subject == subject, EmailThread.sender == sender, EmailThread.recipient == recipient)
...
def get_message_by_thread_and_body(db: Session, thread_id: int, body: str) -> Optional[EmailMessage]:
    return (
        ...
        .filter(EmailMessage.thread_id == thread_id, EmailMessage.body == body)
...
def record_user_decision(
    db: Session, suggestion_id: int, user: str, decision: str, note: str | None = None
) -> UserDecision:
...
def list_messages_for_thread(db: Session, thread_id: int):
    return (
        db.query(EmailMessage).filter(EmailMessage.thread_id == thread_id).order_by(EmailMessage.timestamp.asc()).all()
    )
...
def list_suggestions_for_message(db: Session, message_id: int):
    return (
        ...
        .filter(AISuggestion.message_id == message_id)
...
def has_accepted_decision_for_message(db: Session, message_id: int) -> bool:
    ...
    dec = (
        ...
        .filter(UserDecision.suggestion_id == latest_sug.id, UserDecision.decision == "accept")
        .first()
    )
...
def has_accepted_decision_for_suggestion(db: Session, suggestion_id: int) -> bool:
    dec = (
        db.query(UserDecision)
        .filter(UserDecision.suggestion_id == suggestion_id, UserDecision.decision == "accept")
        .first()
    )
    return dec is not None
...
def create_email_draft(
    ...
    customer_provided: dict | None = None,
    responder_provided: dict | None = None,
    status: str = "draft",
):
    import json as _json
...
def get_latest_draft_for_message(db: Session, message_id: int):
    ...
    return (
        db.query(EmailDraft)
        .filter(EmailDraft.message_id == message_id, EmailDraft.status == "draft")
...
def has_sent_draft_for_message(db: Session, message_id: int) -> bool:
    ...
    d = (
        db.query(EmailDraft)
        .filter(EmailDraft.message_id == message_id, EmailDraft.status == "sent")
